...
{
    LOG_INFO("into http connection's reset function\n");
...
{
    LOG_INFO("pthread: %ld, into to function http_connection::process_read()\n", pthread_self());
...
{
    LOG_INFO("pthread: %ld, into to function get_line()\n", pthread_self());
    memset(current_line, 0, sizeof current_line);
...
http_connection::LINE_STATUS http_connection::parse_line()
{
    ...
    LOG_INFO("pthread: %ld, into to function parse_line()\n", pthread_self());

    int ret = this->input_buffer->buffer_check_line();

    LOG_INFO("the parse_line()'s ret is: %d\n", ret);
...
http_connection::HTTP_CODE http_connection::parse_request_line(char *text)
{
    ...
    LOG_INFO("pthread: %ld, into to function parse_request_line()\n", pthread_self());
...
http_connection::HTTP_CODE http_connection::parse_headers(char *text)
{
    ...
    LOG_INFO("pthread: %ld, into to function parse_headers()\n", pthread_self());

    if (text[0] == '\0')
    {

        LOG_INFO("into here: text[0] == null\n");
...
http_connection::HTTP_CODE http_connection::parse_content(char *text)
{
    ...
    LOG_INFO("pthread: %ld, into to function parse_content()\n", pthread_self());
...
http_connection::HTTP_CODE http_connection::do_request()
{
    ...
    LOG_INFO("pthread: %ld, into to function do_request()\n", pthread_self());

    if (request_method == GET)
    {
        //解析url分析是否有request string，如果有也要运行cgi程序

        LOG_INFO("the http url is %s\n", this->request_url.c_str());
        LOG_INFO("the http version is %s\n", this->request_version.c_str());
        LOG_INFO("the request content length is: %d\n", this->request_content_length);
        LOG_INFO("the linger information is: %s\n", this->if_linger ? "keep-alive" : "close");

        if (request_url.find("?") == std::string::npos)
        { /* 未找到"?",直接请求一个文件 */

            LOG_INFO("have not find '?'\n");
            ...
            if (stat(this->m_file_name.c_str(), &this->m_file_stat) < 0)
            {
                if (ENOENT == errno)
                {
                    LOG_WARNING("the file dose not exist\n");
                }
                else if (ENOTDIR == errno)
                {
                    LOG_WARNING("the file exists but not a real dir\n");
                }
            }
            return FILE_REQUEST;
        }
...
{
    LOG_INFO("pthread: %ld, into to function process_write()\n", pthread_self());
    ...
    case CGI_REQUEST:
    {
        LOG_INFO("into switch case CGI_REQUEST\n");
        append_status_line(200, ok_200_title);
        /* 用于父进程wait与子进程exit */
        int status;
        /* 两个管道 */
        ...
        {
            LOG_ERROR("some wrong when fork");
            return false;
        }
        break;
    }
...
{
    LOG_INFO("pthread: %ld, into to function append_status_line()\n", pthread_self());
    char buf[128];
...
{
    LOG_INFO("pthread: %ld, into to function append_content_type()\n", pthread_self());
    char buf[128];
...
{
    LOG_INFO("pthread: %ld, into to function append_content_length()\n", pthread_self());
    char buf[128];
...
{
    LOG_INFO("pthread: %ld, into to function append_linger()\n", pthread_self());
    char buf[128];
...
{
    LOG_INFO("pthread: %ld, into to function append_blank_line()\n", pthread_self());
    char buf[] = "\r\n";
...
{
    LOG_INFO("pthread: %ld, into to function append_headers()\n", pthread_self());
...
{
    LOG_INFO("pthread: %ld, into to function append_content()\n", pthread_self());
    char str[1024];
...
{
    LOG_INFO("pthread: %ld, into to function process()\n", pthread_self());
    /**
     * 处理读取的数据 
     * 使用状态机
    ...
    if (!write_ret)
    {
        LOG_ERROR("process_write() return false, something wrong\n");
    }
